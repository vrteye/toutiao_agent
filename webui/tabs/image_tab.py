...
def create_image_tab() -> gr.Blocks:
    ...
    with gr.Blocks() as tab:
        ...
        with gr.Row():
            ...
            scene2 = gr.Textbox(label="场景2", interactive=False, visible=False)
            scene3 = gr.Textbox(label="场景3", interactive=False, visible=False)
            scene4 = gr.Textbox(label="场景4", interactive=False, visible=False)

        with gr.Row():
            img1 = gr.Image(label="配图1", height=300)
            img2 = gr.Image(label="配图2", height=300, visible=False)
            img3 = gr.Image(label="配图3", height=300, visible=False)
            img4 = gr.Image(label="配图4", height=300, visible=False)
...
def load_cached_images():
    ...
    scenes = cached.get("scenes", [""] * 4)[:ARTICLE_IMAGE_COUNT]
    while len(scenes) < 4:
        scenes.append("")
    paths = cached.get("image_paths", [])[:ARTICLE_IMAGE_COUNT]
    while len(paths) < 4:
        paths.append(None)
...
def regenerate_images():
    ...
    scenes = article.scenes + [""] * (4 - len(article.scenes))
    ...
    try:
        ...
        for i, scene in enumerate(article.scenes):
            ...
            logger.info(f"[图片] 重新生成 {i+1}/{ARTICLE_IMAGE_COUNT}: {scene[:30]}...")
            ...
            yield scenes[0], scenes[1], scenes[2], scenes[3], *current_paths[:4], f"重新生成中 {i+1}/{ARTICLE_IMAGE_COUNT}..."
        ...
        yield scenes[0], scenes[1], scenes[2], scenes[3], *all_paths[:4], f"重新生成完成: {n_success}/{ARTICLE_IMAGE_COUNT} 张"
